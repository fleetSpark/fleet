...
export const ampAdapter: FleetAdapter = {
  name: 'amp',
  ...
  async start(mission: MissionBrief, context?: string): Promise<AgentSession> {
    ...
    const child = spawn('amp', ['--non-interactive', '--message', prompt], {
      cwd: process.cwd(),
      stdio: ['pipe', 'pipe', 'pipe'],
    });

    if (!child.pid) throw new Error('Failed to start amp process');
    sessions.set(child.pid, child);

    return { pid: child.pid, missionId: mission.id, adapter: 'amp', startedAt: new Date() };
  },
