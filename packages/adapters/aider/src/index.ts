...
export const aiderAdapter: FleetAdapter = {
  name: 'aider',
  ...
  async start(mission: MissionBrief, context?: string): Promise<AgentSession> {
    ...
    const child = spawn('aider', ['--yes-always', '--no-git', '--message', prompt], {
      cwd: process.cwd(),
      stdio: ['pipe', 'pipe', 'pipe'],
    });

    if (!child.pid) throw new Error('Failed to start aider process');
    sessions.set(child.pid, child);

    return { pid: child.pid, missionId: mission.id, adapter: 'aider', startedAt: new Date() };
  },
