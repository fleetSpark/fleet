...
export const geminiAdapter: FleetAdapter = {
  name: 'gemini',
  ...
  async start(mission: MissionBrief, context?: string): Promise<AgentSession> {
    ...
    const child = spawn('gemini', ['--noinput', '--prompt', prompt], {
      cwd: process.cwd(),
      stdio: ['pipe', 'pipe', 'pipe'],
    });

    if (!child.pid) throw new Error('Failed to start gemini process');
    sessions.set(child.pid, child);

    return { pid: child.pid, missionId: mission.id, adapter: 'gemini', startedAt: new Date() };
  },
