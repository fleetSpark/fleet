...
export const codexAdapter: FleetAdapter = {
  name: 'codex',
  ...
  async start(mission: MissionBrief, context?: string): Promise<AgentSession> {
    ...
    const child = spawn('codex', ['--full-auto', '--quiet', prompt], {
      cwd: process.cwd(),
    ...
    return { pid: child.pid, missionId: mission.id, adapter: 'codex', startedAt: new Date() };
  },
